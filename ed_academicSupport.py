...
with open('academic_support_services.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            if row[1]=='United States' and row[5]=='Number' and row[2]!='blank' and row[3] != 'Unknown' and row[3]!= 'Total' and row[4]=='FY 2018':
                ...
                got_support.append(row[3])
                number_support.append(row[6])
            line_count += 1
...
c.execute("DROP TABLE ed_support")
c.execute("CREATE TABLE ed_support(ethnic_group, support_status, count)")

i=0
for data in ethnic_group_2:
    c.execute("INSERT INTO ed_support VALUES (?,?,?)", (ethnic_group_2[i], got_support[i], number_support[i]))
    i+=1
    if i%5==0:
        conn.commit()
...
for group in total_groups:
    ...
    a_yes = 0
    a_no = 0
    r = c.execute("SELECT support_status, count FROM ed_support WHERE support_status='Yes' and ethnic_group=?;",[group])
    ...
    r = c.execute("SELECT support_status, count FROM ed_support WHERE support_status='No' and ethnic_group=?;",[group])
    ...
    percent_received_support.append((a_yes/(a_yes+a_no))*100)

print(ethnic_group_3)
print(percent_enrolled)
print(percent_received_support)
