...
with open('teen_pregnancy.csv') as csv_file:
...
c.execute("DROP TABLE teen_pregnancy")
c.execute("CREATE TABLE teen_pregnancy(ethnic_group, pregnancy, count)")
i=0
for data in ethnic_group:
    c.execute("INSERT INTO teen_pregnancy VALUES (?,?,?)", (ethnic_group[i], status[i], number_pregnancy[i]))
    i+=1
    if i%5==0:
        conn.commit()
...
r = c.execute("SELECT ethnic_group FROM teen_pregnancy")
...
for group in total_groups:
    ...
    count_yes = 0
    count_no = 0
    r = c.execute("SELECT pregnancy, count FROM teen_pregnancy WHERE pregnancy='Yes' and ethnic_group=?;",[group])
    for re in r:
    #     print(re)
        if re[1] !='S':
            count_yes = int(re[1])
    ...
    r = c.execute("SELECT pregnancy, count FROM teen_pregnancy WHERE pregnancy='No' and ethnic_group=?;",[group])
    for re in r:
    #     print(re)
        if re[1] !='S':
            count_no = int(re[1])
